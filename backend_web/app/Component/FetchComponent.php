...
class FetchComponent
{
    ...
    public function add_get($k,$v){$this->gets[]="$k=$v";}

    public function add_post($k,$v){$this->posts[]="$k=$v";}
    ...
    private function load_urlpost($isCleanPost=0)
    {
        if($this->posts)
        {
            if($isCleanPost)
            {
                $this->sUrlPost = http_build_query($this->posts);
            }
            else
            {
                $arParams = [];
    ...
    private function load_curl($isClean=0)
    {
        ...
        if($this->gets)
        {
            if($isClean)
            {
                $this->request_url .= "?".http_build_query($this->gets);
            }
            else
            {
                $sUrl="?";
    ...
    private function curl_execute($options,$sMethod="")
    {
        $this->logd($options,"curl.curl_execute.options");
        $oCurl = curl_init();
        curl_setopt_array($oCurl,$options);
        $this->response = curl_exec($oCurl);
        curl_close($oCurl);
        if($this->response===FALSE)
            $this->add_error("$sMethod.Error FetchComponent.request()");
    }//curl_execute
    ...
    public function request()
    {
        try
        {
            ...
            $this->curl_execute($options,"request");
    ...
    public function request_get($isCleanGet=1)
    {
        try
        {
            ...
            if($this->request_url && $this->gets)
            {
                ...
                $this->curl_execute($options,"request_get");
                $this->log_attribs("request_get");
            }
            else
            {
                $this->add_error("request_get: no urlcurl:$this->request_url or no this->gets");
            }
        }
        catch(Exception $oEx)
        {
    ...
    public function request_fileget($isCleanGet=1)
    {
        try
        {
            ...
            if($this->request_url)
            {
                $this->response = file_get_contents($this->request_url);
                if($this->response===FALSE)
                    $this->add_error("Error en request_fileget");
            }
            else
                $this->add_error("request_fileget: no urlcurl:$this->request_url");
        }
        catch(Exception $oEx)
        {
    ...
    public function request_post($isCleanPost=1)
    {
        try
        {
            ...
            if($this->request_url && $this->sUrlPost)
            {
                $options = array
                (
                    CURLOPT_URL => $this->request_url,
                ...
                $this->curl_execute($options,"request_post");
                $this->log_attribs("request_post");
            }
            else
            {
                $this->add_error("request_post: no urlcurl:$this->request_url");
    ...
    public function request_postrd()
    {
        try
        {
            ...
            if($this->request_url && $this->sUrlPost)
            {
                ...
                $this->curl_execute($options,"request_postrd");
                ...
                preg_match_all("/^Location:(.*)$/mi",$this->response,$arLocation);
            ...
            {
                $this->add_error("request_postrd: no urlcurl:$this->request_url");
    ...
    private function log_attribs($sTitle=NULL)
    {
        ...
        $this->logd($this->request_url,"get");
        $this->logd($this->sUrlPost,"post");
        $this->logd($this->response,"response");
        $this->logd($this->options,"options");
    }//log_attribs()

    public function show_response(){echo $this->response;}
    ...
    public function get_errormsg(){return implode(",",$this->errors);}
    public function get_statusmsg(){return $this->sMessage;}
    public function get_response(){return $this->response;}
    public function get_urlpost(){return $this->sUrlPost;}
    public function get_urlget_final(){return $this->request_url;}
    public function get_urlredir(){return $this->sUrlRedirect;}

    //SETS
    public function is_logdebug($isOn=TRUE){$this->isLogDebug=$isOn;}
    public function add_option($sOption,$sValue){$this->options[$sOption]=$sValue;}
    public function set_options($options){$this->options=$options;}

    public function add_post($sKey,$sValue){$this->posts[$sKey]=$sValue;}
    public function set_post($mxValue){$this->set_array($this->posts,$mxValue);}

    public function add_get($sKey,$sValue){$this->gets[$sKey]=$sValue;}
    public function set_get($mxValue){$this->set_array($this->gets,$mxValue);}

    protected function add_error($sMessage){$this->iserror=TRUE;$this->errors[]=$sMessage;
        $this->log($sMessage,"FetchComponent.error");}
    protected function add_status($response,$sMessage){$this->response=$response;$this->sMessage=$sMessage;}

    public function set_urlget_single($sUrl){$this->request_url=$sUrl;}
    public function set_urlget_final($sUrl){$this->request_url=$sUrl;}
}
